	...
	public function actionSample()
	{
		...
		if (count($resultCdr) > 0) 
		{
			foreach ($resultCdr as $records)
			{
				...
				exec('cp -rf  /var/spool/asterisk/monitor/*.'.$number.'.'.$uniqueid.'.'.$mix_monitor_format.' '.$folder.'/'.$username.'.'.$number.'.'.$uniqueid.'.'.$mix_monitor_format);
			}	
			if ($uniID == true) {
				echo json_encode(array(
					$this->nameSuccess => true,
					$this->nameMsg =>  'http://'.$_SERVER['HTTP_HOST'].'/tmp/'.$username.'.'.$number.'.'.$uniqueid.'.'.$mix_monitor_format
				));
				...
				echo json_encode(array(
					$this->nameSuccess => true,
					$this->nameMsg =>  'http://'.$_SERVER['HTTP_HOST'].'/tmp/records_'.$_SESSION['username'].'.tar.gz'
				));
			}
		...
		else
		{
			echo json_encode(array(
				$this->nameSuccess => false,
				$this->nameMsg =>  'Audio nao encontrado'
			));
			exit;
		}	
